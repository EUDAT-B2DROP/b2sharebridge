<template>
  <div>
    ...
    <b-modal v-if="errormessage !== null" v-model="showErrorModal" id="published_modal" title="B2SHARE" ok-only>
      <div>
        {{ errormessage }}
      </div>
    </b-modal>
    <b-modal v-model="showPublishedModal" id="published_modal" title="B2SHARE" ok-only>
      <div>
        <p class="my-4">Transferring file to B2SHARE in the background.</p>
        <p>Click <a href="/apps/b2sharebridge">here</a> to review the deposit status or edit your draft after the
          transfer.</p>
      </div>
    </b-modal>
  </div>
