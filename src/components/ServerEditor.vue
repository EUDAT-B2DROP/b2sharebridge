<template>
  <div class="server_form">
    ...
    <p>
      <input type="checkbox" :name="getCheckboxName" class="checkbox"
             :checked="mutable_checkSsl" v-model=mutable_checkSsl
             :id="getCheckboxName"
             style="width: 400px">
      <label :for="getCheckboxName">Check SSL</label>
    </p>
    <p id="b2shareUrlField">
      <input title="publishurl" type="text" name="publish_baseurl"
    ...
    <button id="save" @click="saveServer" :disabled="!hasChanged()">Save</button>
    <button v-if="id !== null" @click="deleteServer">Delete</button>
    <button v-if="id === null" id="reset" @click="resetProps" :disabled="!hasChanged()">Reset</button>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    saveServer() {
      ...
      data["publishUrl"] = this.mutable_publishUrl
